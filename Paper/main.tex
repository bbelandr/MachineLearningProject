...
The purpose of the model is to determine whether a person has answered a question correctly while taking a test. The model accomplishes this through features based off a students' answer streak and etc. The algorithm implemented is a combination of logistic regression, random forest classifier, and Extreme Gradient Boost (XGBoost) Classifier combined by voting classifier ensemble method. Accuracy would be balanced with computational efficiency by taking in only a certain number of students for training the models. Results found the each model to be approximately 70\% validation with each model with some models having more overfitting than others (range from 2\% to 9\% difference depending on model).
...
Most of these data are not remotely ready to be used to train a model on. Almost none of the original features that came from this data set were directly used to make our model. For example, the \texttt{user\_answer} feature from KT1 only tells us if the user answers 'A', 'B', 'C', or 'D'. In order to measure the correctness of the user's answer, we have to compare the user's answer with the \texttt{correct\_answer} feature from the \texttt{questions.csv} file. The structure of this data set leaves much work to do to achieve meaningful results. 
...
\subsection{\texttt{elapsed\_time}}
TBD
...
The main purpose of this feature was to profile the students by capturing how well they have been performing so far. This allows the model to tell whether the student is above, below, or plain average without memorizing students, allowing for great generalization on the students in the validation set. We decided to scale \texttt{bayes\_rolling\_acc} with minmax scaling to preserve the small differences in student's performance levels.
...
Due to the large number of tags (\textasciitilde200), with each lecture covering only a single tag, this feature is very sparse. Due to this, we didn't expect the feature to provide a large accuracy boost. Instead, it may help us predict better on some of the tricky scenarios where a student with a low accuracy answers a difficult question correctly. This feature does not need scaling because it is a boolean.
...
\subsection{\texttt{question\_acc}}
TBD
...
\subsection{\texttt{Logistic Regression Model}}
The logistic regression algorithm (Scikit-Learn library) was implemented due to its computational efficiency with large datasets. The model served as a baseline, achieving approximately 71\%. The later inclusion of an SGD classifier was due to logistic regression failing to improve in accuracy with large or nonlinear datasets.

\subsection{\texttt{Scholastic Gradient Descent Classifier (SGD)}}
...
The random forest classifier was chosen for its strong ability to learn nonlinear relationships. While effective, its computational cost became prohibitive for very large datasets. The strategy used to mitigate this is discussed in the Approach section.

\subsection{\texttt{Extreme Gradient Boost Classifier (XGBoost)}}
...
\subsection{\texttt{Voting Ensemble Classifier}}
...
\section{Issues of Overfitting}
Training using only 2000 students led to overfitting, especially for random forest. This was not surprising given the shuffled dataset, which allowed models to easily learn the dominant patterns but occasionally missed patterns appearing less frequently in the full dataset.

\section{Solutions for Overfitting}
To ensure sufficient training data, a logistic regression model was first trained on the entire dataset to establish a reference performance. After testing with 4000-student samples, a 2000-student training sample was found to provide enough data to avoid overfitting (defined as a training–validation accuracy gap of less than 5\%). This approach performed well for logistic regression, SGD, and XGBoost, but not random forest. Hyperparameter tuning was therefore based on F1 score rather than accuracy, as it showed smaller train–validation discrepancies. Validation set size was increased from 200 to 1000, reducing the performance gap by about 5\%. The best models from tuning were then combined using a soft voting classifier—which uses class probabilities rather than majority voting—to produce a more reliable final model.
...
This baseline achieved an overall accuracy of \(\mathbf{65.34\%}\); any useful model must do better than this.
...
This full logistic model achieved a training accuracy of \(\mathbf{73.03\%}\) and a validation accuracy of \(\mathbf{72.92\%}\), which is already a substantial improvement over the 65\% baseline and shows that these simple features contain meaningful signal about student performance.
...
Instead of training on all \(784{,}000\) students, we sampled \(\approx 2000\) students for training and \(\approx 1000\) students for validation via a custom \texttt{read\_in\_data} function.  
...
\begin{itemize}
    \item \textbf{\texttt{question\_acc}}: the empirical accuracy of each question based on training data.
...
Pipeline~2 results are based on \(\approx 2000\) training students and \(\approx 1000\) validation students, while the ``Full Logistic'' model is trained on the full train/validation splits.}
\label{tab:accuracy}
\begin{tabular}{lcc}
\toprule
...
Logistic Regression (Pipeline 2)       & 0.724 & 0.696 \\
Random Forest                          & 0.730 & 0.691 \\
XGBoost                                & 0.721 & 0.695 \\
SGD Classifier                         & 0.715 & 0.688 \\
Voting Ensemble                        & 0.741 & 0.698 \\
\midrule
Logistic Regression (Full Pipeline 1)  & 0.730 & \textbf{0.729} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h!]
    ...
    The model achieved a training accuracy of 72.48\% and a validation accuracy of 69.63\%.  
...
\begin{figure}[H]
    ...
    \caption{Confusion matrix for the Random Forest Classifier. The model achieved a training accuracy of 78.44\% and a validation accuracy of 69.69\%.}
...
\begin{figure}[H]
    ...
    \caption{XGBoost Confusion Matrix. Training accuracy = 72.09\%, Validation accuracy = 69.45\%.}
    \label{fig:xgb_confusion}
\end{figure}

\begin{itemize}
    \item All models in Pipeline 2 beat the 65\% majority baseline by roughly 3--4 percentage points on validation, but the gains from extra model complexity are modest.  
    The best ensemble reaches about \(\approx 69.8\%\) validation accuracy on the sampled students, which is only slightly better than the standalone logistic model in the same pipeline.
    \item The full logistic regression trained on the complete train/validation splits performs best overall (\(\approx 72.9\%\) validation accuracy).  
    This suggests that, for this feature set, simply giving the logistic model more data is more beneficial than stacking more sophisticated learners on a smaller subset.
    ...
    \item The ROC curves provide a more nuanced view: AUC scores are roughly in the 0.70--0.72 range, with Random Forest and XGBoost slightly ahead of logistic regression and SGD.  
...
\begin{itemize}
    \item \textbf{\texttt{question\_acc}} is the single most valuable engineered feature.  
    Comparing models with and without this feature (not shown in the final notebook for brevity), we consistently observed a few percentage points of accuracy gain.  
    ...
    Despite a substantial amount of engineering and tuning, our best ensemble on the sampled pipeline improved accuracy by only about 4 percentage points over the baseline and still underperformed the simple logistic model trained on more data.
\end{itemize}
...
This project set out to evaluate whether student performance on the EdNet-KT1 dataset could be modeled using simple, interpretable features derived from question difficulty and student interaction behavior. Our logistic regression baseline ultimately achieved a training accuracy of 73.03\% and a validation accuracy of 72.92\%, outperforming the majority-class baseline of 65.34\%. Although this gain appears modest, it demonstrates that meaningful predictive structure exists even when using a small number of engineered features.

Several important observations emerged from the modeling process. First, feature engineering mattered far more than model complexity. Early attempts using raw identifiers such as question\_id and bundle\_id added noise without improving accuracy. By contrast, engineered features like question\_accuracy, rolling Bayesian accuracy, forgetful\_ben\_score, and student\_streak have captured interpretable behavioral patterns and consistently improved performance. This aligns with our visual analyses, where distribution plots revealed skewed and heavy-tailed features that benefited significantly from normalization.
...
Third, our logistic regression model provided interpretability benefits that guided future work. The model parameters highlighted which features had the strongest influence on predicting correctness, revealing two insights: (1) elapsed time and question accuracy were the dominant predictors, and (2) identifiers such as question\_id or bundle\_id were not inherently predictive. This informed our decisions when comparing more complex models, showing that additional features must capture meaningful behavior rather than structural metadata.
...
Talk about how much we miss John
